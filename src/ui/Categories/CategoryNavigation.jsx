    ...
    <div className='flex flex-col gap-2 w-full md:w-auto md:flex-row md:flex-wrap justify-center  md:items-center'>
        {
            ...
            localizedCategories.map((category)=>{
                return(
                    <Link to={getLocalizedPath(`/category/${category.id}`,i18n.language)} key={category.id} className={`capitalize p-5 transition-colors duration-300 rounded hover:bg-primary hover:text-white  text-lg  ${id == category.id ? "bg-primary text-white font-bold" :""}`}>{category.title}</Link>
                )
            })
        }
    </div>
