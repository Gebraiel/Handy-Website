      ...
      <div className="flex flex-col md:flex-row gap-10 mb-10">
        {subcategories.length > 0 &&
          subcategories.map((subcategory) => (
            <button
              className={`flex-grow text-center text-2xl font-bold bg-white text-primary shadow-md p-5 ${
                filter == subcategory.id ? "border-t-2" : ""
              }`}
              onClick={() => setFilter(subcategory.id)}
            >
              {subcategory.title}
            </button>
          ))}
      ...
      <div>
        {products.length > 0 ? (
          products
            .filter(
              (product) =>
                product.subcategory?.title.toLowerCase() == filter.toLowerCase()
            )
            .map((product, index) => (
