    ...
    <>
      <FadeIn>
        <Banner image={media_center_cover}>
          <h1 className="text-5xl font-bold text-white">{gallery?.title}</h1>
        </Banner>

        <FadeIn>
          <Section className="!pt-30 !pb-0">
            <SectionTitle className="!mb-10">{t("Media Gallery")}</SectionTitle>
          </Section>
        </FadeIn>
...
export async function loader({ params }) {
  ...
  const { gallery, media_center_cover, error } = await getSingleGallery(
    lang,
    id
  );
